...
#define MAXENV 1024 // max number of environment variables
...
void runMyshell(char* inputString, char** parsedArgs, char** envVars) {
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 4 && !strcmp(argv[1], "--server") && !strcmp(argv[2], "--port")) {
        ...
        for (;;) {
            ...
            if ((cfd = accept(sfd, NULL, NULL)) == -1) exit(1);


            cpid = fork();
            if (cpid == -1) {
                std::cerr << "Failed forking child.." << std::endl;
                status = -1;
                exit(EXIT_FAILURE);
            } else if (cpid == 0) {
                ...
                while (1) {
                    ...
                    int cc = recv(cfd, buf, sizeof(buf), 0);
                    if (cc == 0) exit(EXIT_SUCCESS);
                    buf[cc - 2] = '\0';

                    runMyshell(buf, parsedArgs, envVars);
    ...
    while (1) {
        ...
        runMyshell(inputString, parsedArgs, envVars);
